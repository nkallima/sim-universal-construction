...
void start_cpu_counters(int id) {
    ...
    if (PAPI_add_event(__cpu_events, PAPI_L1_DCM) != PAPI_OK) {
       fprintf(stderr, "PAPI ERROR: unable to create event L1 data cache misses\n");
       exit(EXIT_FAILURE);
    }
    if (PAPI_add_event(__cpu_events, PAPI_L2_DCM) != PAPI_OK) {
       fprintf(stderr, "PAPI ERROR: unable to create event L2 data cache misses\n");
       exit(EXIT_FAILURE);
    }
    if (PAPI_add_event(__cpu_events, 0x40000011) != PAPI_OK) {
       fprintf(stderr, "PAPI ERROR: unable to create event L3 cache misses\n");
       exit(EXIT_FAILURE);
    }
    if (PAPI_add_event(__cpu_events, PAPI_RES_STL) != PAPI_OK) {
       fprintf(stderr, "PAPI ERROR: unable to create event stalls\n");
       exit(EXIT_FAILURE);
    }
    if (PAPI_start(__cpu_events) != PAPI_OK) {
       fprintf(stderr, "PAPI ERROR: unable to start counter\n");
       exit(EXIT_FAILURE);
    }
#endif
}
